...
internal class InferenceFilter(ModelConfig modelConfig, ILogger<InferenceFilter> logger)
{
    ...
    private readonly ConcurrentDictionary<string, long> confinedSpaceProcessedCoordinates = [];
    ...
    internal bool IfCoordinatesForConfinedSpaceIsProcessed(string cameraSerial, long timestamp)
    {
        // If the inference is not present in confinedSpaceProcessedCoordinates dict, we will add the current timestamp to the dict and we will not send the alert and return false (because we are having a cooling period)
